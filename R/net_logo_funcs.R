...
# With variables 
make_experiment_xml <- function(variables, values, metric,repetitions=1, stepLimit=500){
  ...
  timeLimit <- newXMLNode("timeLimit", attrs = c(steps=stepLimit))
...
make_experiments_doc <- function(variables, values, metric,repetitions=1, stepLimit=50){
  experiment_xml <-make_experiment_xml(variables, values, metric,repetitions, stepLimit)
...
construct_nl_command<-function(nl_path, model_path, experiment_path,
                               jar_name="NetLogo.jar",
                               out_path=F){
  jar_path <- shQuote(file.path(nl_path, jar_name))
...
make_run_model_command <- function(nl_path, model_path, experiment_path, 
                                   metric,variables, stepLimit=500,
                                   jar_name="Netlogo.jar"){
  runModel <- function(..., run_no=1){
    values <- c(...)
    prefix <- '<?xml version="1.0" encoding="us-ascii"?>'
    exp_doc <- make_experiments_doc(variables, values, metric ,stepLimit=stepLimit)
